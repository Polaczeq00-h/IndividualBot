...
  
    // Logowanie typu kanału dla celów diagnostycznych:
    if (i.channel.type === 1) { 
...
  }
    ...
    // Walidacja użycia na serwerze: jeśli komenda wymaga celu, ale go nie podano na serwerze, odrzuć.
    if (!i.inGuild() && !user && name !== 'co' && name !== 'rozkurw' && name !== 'impreza' && name !== 'los') {
        // Jeśli jesteśmy w DM i nie podano 'kto', targetUser to i.user, więc bot odpowiada samemu sobie. W DM jest to OK.
        // Ta walidacja jest dla serwerów, ale i tak nie jest potrzebna, bo targetUser jest zawsze zdefiniowany.
    }
